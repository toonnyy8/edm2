...
def _edm_sampler(
    ...
    # Guided denoiser.
    def denoise(x, t):
        Dx, logvar = net(x, t, labels, return_logvar=True)
        Dx, logvar = Dx.to(dtype), logvar.to(dtype)
        if guidance == 1:
            return Dx, logvar.exp().sqrt()
        ref_Dx = gnet(x, t, labels).to(dtype)
        return ref_Dx.lerp(Dx, guidance), logvar.exp().sqrt()
    ...
    prev_x = torch.zeros_like(x_next)
    _prev_x = torch.zeros_like(x_next)
    # prev_x = torch.from_numpy(np.load(".datasets/cifar_mean.npy")).to(noise.device) # torch.zeros_like(x_next)
    # prev_std = torch.from_numpy(np.load(".datasets/cifar_std.npy")).to(noise.device) # torch.zeros_like(x_next)
    # print(prev_std)
    # prev_x = torch.zeros_like(x_next)
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        # Euler step.
        x, std = denoise(x_hat, t_hat)
        _x = x # (1 + w) * x - w * prev_x
        d_cur = (x_hat - _x) / t_hat
        x_next = x_hat + (t_next - t_hat) * d_cur
        prev_x = x

        # Apply 2nd order correction.
        if i < num_steps - 1:
            x, _ = denoise(x_next, t_next)
            _x = x # (1 + w) * x - w * _prev_x
            d_prime = (x_next - _x) / t_next
            # x_next = x_hat + (t_next - t_hat) * (0.5 * d_cur + 0.5 * d_prime)
            x_next = x_hat + (t_next - t_hat) * ((t_hat-t_next)/(t_hat) * d_cur + t_next/(t_hat) * d_prime)
            _prev_x = x
